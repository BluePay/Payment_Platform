...
BluePayPayment_BP10Emu payment = new BluePayPayment_BP10Emu(
  ...
  MODE);
    ...
    BluePayPayment_BP10Emu paymentToken = new BluePayPayment_BP10Emu(
    ACCOUNT_ID,
    SECRET_KEY,
    MODE);
    
    // Attempts to capture above Auth transaction
    paymentToken.capture(payment.getTransID());
    try
        {
    ...
    if(paymentToken.isError())
        {
          System.out.println("Error: " + paymentToken.getMessage());
        }
        else if(paymentToken.isApproved() || paymentToken.isDeclined())
        {
